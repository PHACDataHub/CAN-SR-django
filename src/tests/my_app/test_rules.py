...
def test_review_access_rule(admin_user):
    ...
    linked_review = ReviewFactory()
    other_review = ReviewFactory()
    ReviewUserLinkFactory(user=linked_user, review=linked_review)

    with GlobalRequest():
        assert my_test_rule("can_access_review", admin_user, linked_review.id)
        assert my_test_rule("can_access_review", linked_user, linked_review.id)
        assert not my_test_rule(
            "can_access_review", linked_user, other_review.id
        )
        assert not my_test_rule(
